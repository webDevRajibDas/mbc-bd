...
<form method="POST" action="{{ route('geographic.unions.update', $union) }}">
    @csrf
    @method('PUT')
    ...
    <div class="col-md-12">
        <div class="tile">
            <div class="tile-title-w-btn">
                <h3 class="title">Edit Union: {{ $union->name }}</h3>
                <p>
                    ...
                    <div class="col-lg-6">
                        <div class="form-group mb-3">
                            ...
                            <select name="upazila_id" id="upazila_id" class="form-control form-control-sm select2" required>
                                ...
                                @foreach($upazilas as $upazila)
                                    <option value="{{ $upazila->id }}" {{ old('upazila_id', $union->upazila_id) == $upazila->id ? 'selected' : '' }}>
                        ...
                        <div class="form-group mb-3">
                            ...
                            <input type="text" name="name" id="name" class="form-control form-control-sm"
                                   placeholder="Enter Union Name" required value="{{ old('name', $union->name) }}">
                            @error('name')
                                <small class="text-danger">{{ $message }}</small>
                            @enderror
                        </div>
                        ...
                        <div class="form-group mb-3">
                            ...
                            <input type="text" name="bn_name" id="bn_name" class="form-control form-control-sm"
                                   placeholder="Enter Bengali Name" value="{{ old('bn_name', $union->bn_name) }}">
                            @error('bn_name')
                                <small class="text-danger">{{ $message }}</small>
                            @enderror
                        </div>
                        ...
                        <div class="form-group mb-3">
                            ...
                            <input type="text" name="url" id="url" class="form-control form-control-sm"
                                   placeholder="Enter website URL (e.g., https://example.com)" value="{{ old('url', $union->url) }}">
                            @error('url')
                                <small class="text-danger">{{ $message }}</small>
                            @enderror
                        </div>
