...
<form method="POST" action="{{ route('geographic.districts.update', $district) }}">
    @csrf
    @method('PUT')
    ...
    <div class="col-md-12">
        <div class="tile">
            <div class="tile-title-w-btn">
                <h3 class="title">Edit District: {{ $district->name }}</h3>
                <p>
                    ...
                    <div class="col-lg-6">
                        ...
                        <div class="form-group mb-3">
                            ...
                            <input type="text" name="name" id="name" class="form-control form-control-sm"
                                   placeholder="Enter District Name" required value="{{ old('name', $district->name) }}">
                            @error('name')
                                <small class="text-danger">{{ $message }}</small>
                            @enderror
                        </div>
                        ...
                        <div class="form-group mb-3">
                            ...
                            <input type="text" name="bn_name" id="bn_name" class="form-control form-control-sm"
                                   placeholder="Enter Bengali Name" value="{{ old('bn_name', $district->bn_name) }}">
                            @error('bn_name')
                                <small class="text-danger">{{ $message }}</small>
                            @enderror
                        </div>
                        ...
                        <div class="form-group mb-3">
                            ...
                            <input type="text" name="url" id="url" class="form-control form-control-sm"
                                   placeholder="Enter website URL (e.g., https://example.com)" value="{{ old('url', $district->url) }}">
                            @error('url')
                                <small class="text-danger">{{ $message }}</small>
                            @enderror
                        </div>
